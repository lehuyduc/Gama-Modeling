	...
	output {
		...
		display R0_chart {
			chart "R0 values" type: series style: line {
				...
				value: [human where(each.isInfected) min_of(each.myR0), human where(each.isInfected) max_of(each.myR0), averageR0]
		...
		display Population_gender {
			chart "Population gender" type: histogram {
				datalist ["Male", "Female"] value:[length(human where(each.gender="male")), length(human where(each.gender="female"))];
			}								
		}
		
		display Population_by_age {
			chart "Population by age" type: histogram {
				datalist ["0-3", "4-22", "23-55", "55+"] 
				value:[length(human where(each.age<=3)), 
					   length(human where(3<each.age and each.age<=22)),
					   length(human where(22<each.age and each.age<=55)), 
					   length(human where(55<each.age))
					  ];
			}
		}
...
experiment E2_34  {
	...
	parameter "Initial infected" var: initialInfected init:10 min:1 max:100;	
	parameter "Contagion in building" var: contagionInBuilding init: false among:[true, false];
	
	init {
		create simulation with:[humanNumber::humanNumber, initialInfected::initialInfected, contagionInBuilding::true, seed::seed];		
	}
	...
	output {
		...
		monitor "Hour:" value: currentHour;
		monitor "Day:" value: currentDay;		
		monitor "Contagion in building" value: contagionInBuilding;
	}
}